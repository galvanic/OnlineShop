...
"""
Script to automate who owes what for an online shop (i.e. easily dividing the bill).
...
def get_latest_file(file_extension, directory=SHOP_DIRECTORY):
    ...
    the given directory.
    """
    filenames = [fn for fn in os.listdir(directory) if fn[-3:] == file_extension]
    newest = max(filenames, key=lambda x: os.stat(x).st_mtime)
    return '{}{}'.format(directory, newest)

def choose_file(directory=SHOP_DIRECTORY):
    """
    CLI to choose a file in given directory to use as shop receipt text
    OR as already parsed and assigned items.
    """
    filenames = [fn for fn in os.listdir(directory) if fn[-3:] in ["txt", "csv"]]
    for i, filename in enumerate(filenames, 1):
        print(i, filename)
    chosen = ask("Which file do you want to open ?", range(1, len(filenames)+1))
    if not chosen:   # this doesn't work for the moment, can't figure out how to make it accept a None value
        filename = get_latest_file("csv")
    filename = filenames[int(chosen)-1]
    return '{}{}'.format(directory, newest)

def find_shop_items(filename, write2file=True):
    """
    ...
    if write2file:
        write_shop_to_file(shop_items, "Ocado Shop %s.csv"%(delivery_date))

    return shop_items

def write_shop_to_file(shop_items, ofilename="temp_shop.csv", verbose=False):
...
def get_shop_items(filename):
    """
    From a csv file,
    Returns an (ordered) list of ShopItems.     # ordered by what ??
    """
    if filename[-3:] == "txt":
        return find_shop_items(filename)
...
def display_shop(itemlist, item_range=None):
    """
...
def ask_whose(shop_items, start_line=1):
    """
...
def is_every_item_assigned(shop_items):
    for i, item in enumerate(shop_items):
...
def calculate_money_owed(shop_items):
    """"""
    # check every item is assigned otherwise, division by zero next
    if not is_every_item_assigned:
...
def main():
    shopfile = choose_file()
    shop_items = get_shop_items(shopfile)
    display_shop(shop_items)
    line_start = ask("Which line do you want to start at ?", range(len(shop_items)))
    shop_items = ask_whose(shop_items, int(line_start))
    write_shop_to_file(shop_items, verbose=True)
    display_shop(shop_items)
    calculate_money_owed(shop_items)
    return
